...
def main():
    ...
    if wmt:
        ...
        A("")
        ablation = REPO / "results/wmt14_small/beam_ablation.md"
        if ablation.exists():
            # Generated by scripts/analyze_beam.py from the decoded hypotheses.
            A(ablation.read_text().rstrip())
            A("")
    ...
    A("under an otherwise identical setup, and the ordering of Table 1's rows 3 and 4 holds at our")
    A("scale. The reversed model was ahead at *every* epoch, not merely at the end.")
    A("")
    A("**Not reproduced — and reported rather than hidden.** The paper's monotone improvement with")
    A("beam size does not survive at our scale: BLEU peaks at beam 2 and falls at beam 12. Section 5")
    A("decomposes this. Length normalisation removes the brevity penalty but makes BLEU *worse*, so")
